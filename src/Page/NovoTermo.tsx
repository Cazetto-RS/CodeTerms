...
export default function NovoTermo({ onNavegar }: { onNavegar: (pagina: any) => void }) {
  const { width } = useWindowDimensions();
  const s = getStyles(width);
  ...
  const handleSalvar = async () => {
    ...
    if (!termo || !traducao || !definicao) {
      setErro("Termo, tradução e definição são obrigatórios."); return;
    }
    ...
    return (
      <ScrollView style={s.scroll} contentContainerStyle={s.center}>
        <View style={s.card}>
          ...
          <Text style={s.cardSubtitle}>O novo termo foi salvo com sucesso no dicionário.</Text>
          <TouchableOpacity style={s.btn} onPress={() => { setSucesso(false); setTermo(""); setTraducao(""); setDefinicao(""); setPronuncia(""); setSilabas(""); setSinonimos(""); setAntonimos(""); setExemploEn(""); setExemploPt(""); setImagem(null); }}>
  ...
  return (
    <ScrollView style={s.scroll} contentContainerStyle={s.center}>
      <View style={s.card}>
        ...
        <View style={s.cardIcon}><PlusIcon /></View>
        <Text style={s.cardTitle}>Novo termo</Text>
        <Text style={s.cardSubtitle}>Preencha os dados do novo termo do dicionário.</Text>
        ...
        {/* Obrigatórios */}
        <Campo label="Termo *" placeholder="Ex: Array" value={termo} onChangeText={setTermo} />
        <Campo label="Tradução *" placeholder="Ex: Arranjo" value={traducao} onChangeText={setTraducao} />
        ...
        <View style={s.divider}>
          ...
          <Text style={s.dividerText}>Opcionais</Text>
          <View style={s.dividerLine} />
        </View>

        <Campo label="Pronúncia" placeholder="Ex: /əˈreɪ/" value={pronuncia} onChangeText={setPronuncia} />
        <Campo label="Sílabas" placeholder="Ex: ar·ray" value={silabas} onChangeText={setSilabas} />
        <Campo label="Exemplo em inglês" placeholder="Ex: Create an array of numbers." value={exemploEn} onChangeText={setExemploEn} />
        <Campo label="Exemplo em português" placeholder="Ex: Crie um arranjo de números." value={exemploPt} onChangeText={setExemploPt} />
        <Campo label="Sinônimos" hint="Separe por vírgula. Ex: List, Collection" placeholder="Ex: List, Collection" value={sinonimos} onChangeText={setSinonimos} />
        <Campo label="Antônimos" hint="Separe por vírgula." placeholder="Ex: Scalar" value={antonimos} onChangeText={setAntonimos} />

        {/* Upload de imagem */}
        <View style={s.fieldGroup}>
          <Text style={s.label}>Imagem</Text>
          <label style={{ display: "block" }}>
            <View style={[s.input, { alignItems: "center", justifyContent: "center", height: 44, cursor: "pointer" } as any]}>
              <Text style={{ fontSize: 13, color: imagem ? "#131417" : "#9CA3AF" }}>
                {imagem ? imagem.name : "Selecionar imagem (JPG, PNG)"}
              </Text>
            </View>
            <input type="file" accept="image/jpg,image/jpeg,image/png" style={{ display: "none" }} onChange={handleImagem} />
          </label>
        </View>
